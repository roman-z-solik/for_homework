...
**interface.py** содержит 8 функций:

`user_input_y_n`    
Функция, определяющая, ответил пользователь 
ДА или НЕТ на вопрос.  

`import_data`  
Функция предоставляет выбор пользователю, из какого файла импортировать
данные о банковских операциях: JSON, CSV или XLSX и 
...
статусы: [E]XECUTED, [C]ANCELED, [P]ENDING. Поддерживается ввод 
необходимого статуса одной буквой.  

`sort_re`  
Функция принимает список словарей с данными о банковских операциях и 
данные для поиска, после чего возвращает список отсортированных с 
помощью библиотеки re словарей.

`sort`
Функция принимает список словарей с данными о банковских операциях и 
данные для сортировки, после чего возвращает список отсортированных 
словарей.

`count_categories`
Функция подсчета количества операций d отсортированном списке.
